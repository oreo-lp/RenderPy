...
class Triangle(object):
	...
	def contains_point(self, point):
		...
		if (alpha >= 0 and beta >= 0 and gamma >= 0):
			...
			transparency = int(alpha*self.p0.color.a() + beta*self.p1.color.a() + gamma*self.p2.color.a())

			# Also interpolate the z-value of this point
			zValue = alpha * self.p0.z + beta * self.p1.z + gamma * self.p2.z
			return True, Color(red, green, blue, transparency), zValue
	...
	def draw_faster(self, image, zBuffer):

		# Calculate bounding box for this triangle
		ymin = max(min(self.p0.y, self.p1.y, self.p2.y), 0)
		ymax = min(max(self.p0.y, self.p1.y, self.p2.y), image.height - 1)

		# Iterate over all scan lines in the bounding box
		for y in range(ymin, ymax + 1):
			# Find intersections of the scan line with triangle edges
			x_values = []
			for edge_start, edge_end in [(self.p0, self.p1), (self.p1, self.p2), (self.p2, self.p0)]:
				if (edge_start.y <= y <= edge_end.y) or (edge_end.y <= y <= edge_start.y):
					if edge_end.y == edge_start.y:  # Check if the edge is horizontal
						continue
					if edge_start.y == y or edge_end.y == y:
						if edge_start.y == y and edge_start.x not in x_values:
							x_values.append(edge_start.x)
						if edge_end.y == y  and edge_end.x not in x_values:
							x_values.append(edge_end.x)
					else:
						t = (y - edge_start.y) / (edge_end.y - edge_start.y)
						x = int(edge_start.x + t * (edge_end.x - edge_start.x))
						x_values.append(x)
			if len(x_values) > 0:
				# Sort intersections and clamp to image bounds
				if len(x_values) == 1:
					x_start = x_values[0]
					x_end = x_start
				else:
				...
				x_end = min(x_end, image.width - 1)

				# Iterate over horizontal span of the scan line within the triangle
				for x in range(x_start, x_end + 1):
					point = Point(x, y, color=None)
					point_in_triangle, color, zValue = self.contains_point(point)
					if point_in_triangle:
						# Check z-buffer to determine whether to draw this pixel
						if zBuffer[y * image.width + x] < zValue:
							zBuffer[y * image.width + x] = zValue
